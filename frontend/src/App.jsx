...
+/)
                  ...
                  return (
                    <Accordion key={i} title={sec.title} isPaid={true} defaultOpen={i === 0} content={
                      <span>
                        {firstPara}
                        <div style={{ marginTop: 16, background: 'linear-gradient(135deg, #F3EEFF, #EEF4FF)', border: '1.5px dashed #C4B5FD', borderRadius: 10, padding: '14px 16px' }}>
                          <p style={{ fontSize: 13, color: '#7C3AED', fontWeight: 700, marginBottom: 6 }}>🔒 지금 당장 실천할 수 있는 조언</p>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {['첫째, ████████████████████', '둘째, ████████████████████', '셋째, ████████████████████'].map((t, idx) => (
                              <p key={idx} style={{ fontSize: 13, color: '#A78BFA', margin: 0, filter: 'blur(3.5px)', userSelect: 'none' }}>{t}</p>
                            ))}
                          </div>
                          <p style={{ fontSize: 12, color: '#7C3AED', marginTop: 10, fontWeight: 600 }}>👆 전체 분석에서 확인하세요</p>
                        </div>
                      </span>
                    } />
                  )
                }
                return <Accordion key={i} title={sec.title} content={sec.content} isPaid={true} defaultOpen={i === 0} />
              })}
              {행운아이템 && (
