       ...
       {!isStep0 && !isStep1 && (
            <div style={{ marginTop: 32, background: 'linear-gradient(135deg, #0D1B3E 0%, #050D1F 100%)', border: '1px solid rgba(155,29,58,0.4)', borderRadius: 16, padding: '24px 20px', marginBottom: 8 }}>
              ...
              {(() => {
                ...
                return [
                  { title: isLover ? '성격 궁합' : '성격 케미', preview: 관계유형 === '친구' ? '이 친구, 내가 힘들 때 진짜 내 편이 돼줄 사람인지 사주에서 보여요.' : '두 사람은 겉으로 보기엔 달라 보이지만, 사주 구조상 서로의 빈자리를 채워주는 관계예요. 두 사람 사이에서 반복되는 패턴이 있어요. 이게 뭔지 알면 관계가 달라지는데, 지금은 딱 그 직전이에요.', blurred: '한 사람이 불을 지피면 다른 한 사람이 방향을 잡아주는 구조라 함께할수록 시너지가 나요. 단, 속도 차이에서 오는 충돌이 생길 수 있고 이걸 어떻게 다루느냐가 관건이에요. 특히 감정 표현 방식이 정반대라 한쪽은 말로 풀려 하고 다른 한쪽은 행동으로 보여주려 해요. 이 차이를 이해하면 싸움이 줄고, 모르면 같은 패턴이 반복돼요. 두 사람의 일간 조합에서 목화(木火)의 기운이 강하게 작용하고 있어서 초반 끌림은 강하지만 유지하려면 서로의 리듬을 맞추는 연습이 필요해요. 실제로 이 조합은 3년 차에 가장 큰 고비가 오는데, 그 시기를 어떻게 넘기느냐가 이 관계의 수명을 결정해요.' },
                  { title: 관계유형 === '연인' ? '돈 궁합' : '재물 운', preview: 관계유형 === '친구' ? '오래된 인연일수록 사주 구조가 맞는지가 중요해요. 이 두 사람이 계속 가까울 수 있는 구조인지 나와요.' : '두 사람이 함께 돈을 모을 수 있는 구조인지, 아니면 한 사람이 새는 구조인지 나와요.', blurred: '두 사람이 같이 있을 때 돈이 모이는 구조인지, 아니면 쓰게 되는 구조인지 사주에 다 나와요. 공동 투자나 재정 운용 방향도 확인할 수 있어요. 한 사람은 모으는 데 강하고 다른 한 사람은 불리는 데 강한 구조인데, 이 역할 분담이 자연스럽게 맞아떨어지면 재산이 빠르게 늘어나는 조합이에요. 단, 큰 지출에 대한 기준이 다를 수 있어서 미리 합의하지 않으면 돈 문제로 감정이 상할 수 있어요. 특히 부동산이나 목돈이 움직이는 시기에 의견 충돌이 생기기 쉬운 구조예요. 두 사람이 함께 투자하면 안 되는 분야가 따로 있고, 반대로 같이 하면 시너지가 나는 재테크 방향도 구체적으로 나와요. 공동 재정을 운용할 때 절대 하면 안 되는 실수 한 가지도 짚어드려요.' },
              ...
              <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', textAlign: 'center', marginTop: 12 }}>{관계유형 === '친구' ? '두 사람의 우정이 어디까지 가는지 전부 나와요 👇' : '두 사람의 결혼·돈·미래 궁합까지 전부 나와요 👇'}</p>
            </div>
          )}
        </div>
        <div style={{ position: 'fixed', bottom: 0, background: '#050D1F', borderTop: '1px solid rgba(201,168,76,0.15)', padding: '12px 16px 24px', display: 'flex', gap: 10, maxWidth: 480, width: '100%', left: '50%', transform: 'translateX(-50%)', boxSizing: 'border-box', zIndex: 100 }}>
