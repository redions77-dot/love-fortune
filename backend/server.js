...
- 함께 더 잘 살아가는 조언
현실적이고 따뜻하게, 두 사람이 함께 더 행복해지는 방향으로 작성해주세요.`,
        paidFocus: `===부부운 심화 분석===
...
- 전 결혼과는 어떻게 다를지
- 새로운 시작을 위한 사주의 메시지
판단하지 않고, 새 출발에 힘을 주는 따뜻한 톤으로 작성해주세요.`,
        paidFocus: `===재혼운 심화 분석===
...
- 앞으로의 인연운과 재혼 가능성
- 이 사주의 진짜 강점과 행복의 방향
절대 비판하지 않고, 깊이 공감하며 새로운 시각을 주는 톤으로 작성해주세요.`,
        paidFocus: `===결혼 패턴 심화 분석===
...
아래 3개 섹션을 순서대로 작성해주세요. 각 섹션은 ===섹션제목=== 형태로 구분하고, 마크다운 없이 일반 텍스트로만 작성하세요.

===나의 사주 기운===
이 사람의 일간 기운과 전체 사주 특성을 따뜻하게 설명해주세요. (400~500자)

===재물운 맛보기===
이 사주의 재물운 특징을 간략히 알려주세요. 돈과 어떤 관계인지, 재물이 들어오는 방식의 특징. (200~300자)
마지막 줄은 반드시: "💰 더 자세한 재물운·직업운은 전체 분석에서 확인하세요"
...
아래 6개 섹션을 순서대로 작성해주세요. 각 섹션은 ===섹션제목=== 형태로 구분하고, 마크다운 없이 일반 텍스트로만 작성하세요.
각 섹션은 300~500자로 구체적이고 학부모 입장에서 실질적으로 도움이 되게 작성해주세요.

===타고난 공부 기질===
이 아이가 타고난 공부 머리와 지적 특성을 분석해주세요.

===재능이 빛나는 분야===
특히 두각을 나타낼 과목이나 분야 (이과/문과 성향, 예체능 등)를 분석해주세요.

===학습 에너지가 높아지는 시기===
집중력과 학습 에너지가 높아지는 시기와 나이를 분석해주세요.

===입시 운 흐름===
입시 운이 좋은 해와 주의할 시기를 분석해주세요.

===부모가 도와주는 법===
부모와 자녀 사주의 연결고리를 보며 부모가 어떻게 도와줄 수 있는지 분석해주세요.

===공부 잘 되는 환경===
이 아이가 공부를 잘 하는 환경과 방법을 분석해주세요.`;
