...
두 번째 문단: 지금 이 시기 학업 에너지의 흐름이 어떤지만 알려주세요. 현재 시기가 공부하기 좋은 때인지, 조심할 때인지.
...
첫 문단: 이 사람이 끌리는 유형과 실제로 잘 맞는 유형의 차이, 연애에서 반복하는 패턴을 정확하게 찌르세요. 이 사람에게만 해당하는 내용이어야 해요.
...
두 번째 문단: 지금 이 시기 부부 관계의 흐름 방향만 알려주세요.
마지막 줄: "💍 부부운 상세·갈등 시기·가정운은 전체 분석에서 확인하세요"`;
  }

  return `===지금 이 시기 핵심운===
이 사람은 현재 ${age}세입니다. (250~300자)
첫 문단: 지금 이 나이에 이 사주에서 가장 중요한 흐름이 무엇인지 정확하게 찌르세요. 이 사람의 사주 구조에서 실제로 두드러지는 특징이어야 해요.
두 번째 문단: 현재 시기가 이 사람에게 어떤 의미인지 방향만 알려주세요. 구체적 조언은 하지 마세요.
마지막 줄: "🔮 월별 흐름·직업·투자 타이밍은 전체 분석에서 확인하세요"`;
}
...
두 번째 문단: 입시나 취업에서 유리한 구체적 시기, 현재 학업·진로 흐름을 설명하세요. 이 사람의 사주 구조에서 실제로 보이는 흐름을 기준으로 써주세요.`;
  }
...
두 번째 문단: 이 사주가 결혼으로 이어지기 위한 조건, 가까운 시기의 인연운 흐름을 설명하세요. 이 사람의 사주에서 실제로 보이는 내용만 써주세요.`;
    }
    return `===부부운 심화===
무료에서 쓴 내용은 반복하지 마세요. (500~600자)
첫 문단: 앞으로 부부 사이에 좋은 시기와 갈등이 생기기 쉬운 구체적 시기를 설명하세요.
두 번째 문단: 배우자 복이 있는 사주인지 상세 분석, 가정운·자녀운의 흐름을 설명하세요.`;
  }

  return `===앞으로의 핵심 흐름===
무료에서 쓴 내용은 반복하지 마세요. (500~600자)
첫 문단: 앞으로 5~10년 안에 이 사람에게 중요한 변화가 예상되는 시기와 그 내용을 구체적으로 설명하세요. 이 사람의 사주 구조에서 실제로 보이는 것만 써주세요.
두 번째 문단: 지금 이 시기를 어떻게 보내는 것이 이 사주에 맞는지 구체적으로 설명하세요.`;
}

// 공통 규칙 — 호칭 통일 + 2026년 강조 제거 + 차별화 강조
const 공통규칙 = `작성 규칙 (반드시 지킬 것):
1. 호칭: 분석 대상을 항상 "당신"으로만 표현하세요. "형님", "누나", "어르신", "선생님" 등 다른 호칭은 절대 사용 금지
2. 쉬운 말: 어려운 한자 용어 금지. 예: "인성" → "나를 도와주는 기운", "식신" → "타고난 재능"
3. 나열 금지: 숫자 목록(1. 2. 3.)이나 하이픈(-) 금지 — 자연스러운 문장으로
4. 마크다운 금지: **, ##, * 등 기호 사용 금지
5. 문단 구성: 첫 문단은 공감 가는 설명, 둘째 문단은 사주 근거
6. 차별화: 이 사람의 사주에서 실제로 두드러지는 특징만 써주세요. 누구에게나 해당되는 일반적인 말은 쓰지 마세요
7. 연도 언급: 현재는 2026년입니다. 2026년이 이 사주에서 실제로 중요한 해일 때만 언급하세요. 중요하지 않으면 굳이 언급하지 마세요
8. 유행어·밈: 전체에서 최대 1개`;
...
- 년주: ${년주} / 월주: ${월주} / 일주: ${일주} / 시주: ${시주 || '미입력'}`;
  ...
  if (type === '궁합') {
    ...
    if (partnerIsLunar) {
      try {
        const solar = lunarToSolar(pYear, pMonth, pDay);
        pYear = solar.year; pMonth = solar.month; pDay = solar.day;
      } catch (e) { console.error('음력 변환 오류:', e); }
    }

    const pBirthdate_solar = `${pYear}-${String(pMonth).padStart(2,'0')}-${String(pDay).padStart(2,'0')}`;
    const p일주obj = get일주(pBirthdate_solar);
    const p년주obj = get년주(pYear);
    const p일주 = p일주obj.간지;
    const p년주 = p년주obj.간지;
    const p월주 = get월주(pYear, pMonth, pDay, p년주obj.천간index);
    const p시주 = get시주(partnerBirthtime, p일주obj.천간index);
...
[B (상대방)]
- 성별: ${partnerGender || '미입력'} / 생년월일: ${pYear}년 ${pMonth}월 ${pDay}일 (${2026 - pYear}세)
- 년주: ${p년주} / 월주: ${p월주} / 일주: ${p일주} / 시주: ${p시주 || '미입력'}

아래 4개 섹션을 ===섹션제목=== 형태로 작성하세요. 각 섹션 300~400자.
두 사람을 각각 "A"와 "B"로 구분해서 표현하세요.

===성격 궁합===
두 사람의 성격이 어떻게 만나는지 분석해주세요.
잘 맞는 부분과 부딪히는 부분, 서로에게 어떤 존재가 되는지를 솔직하게 써주세요.
좋은 말만 하지 말고, 실제로 갈등이 생길 수 있는 지점도 정확하게 찌르세요.

===돈 궁합===
두 사람이 함께하면 재물이 늘어나는 구조인지 줄어드는 구조인지 분석해주세요.
각자의 재물 스타일이 만났을 때 시너지가 나는지, 서로의 돈을 갉아먹는 구조인지.
함께 돈을 모으려면 어떤 방식이 맞는지도 알려주세요.

===결혼 궁합===
이 두 사람이 함께 살면 서로에게 도움이 되는 사주인지 솔직하게 분석해주세요.
결혼 후 관계가 어떻게 흘러가는지, 서로가 서로에게 어떤 영향을 주는지.
조심해야 할 시기나 패턴이 있다면 구체적으로 알려주세요.

===궁합 총평===
이 두 사람의 궁합을 한 마디로 표현하고, 잘 살기 위한 핵심 조언을 주세요.
궁합 점수를 100점 만점으로 표현하고 (예: 74점), 그 이유를 설명해주세요.
마지막 줄: 이 두 사람에게 가장 중요한 한 가지 조언을 한 문장으로 써주세요.`;
...
첫 문단: 이 사람을 딱 한 마디로 표현하는 이미지로 시작해서, 겉으로 보이는 모습과 속마음의 차이, 주변 사람들이 이 사람을 어떻게 느끼는지를 정확하게 써주세요. 이 사람의 사주 구조에서 실제로 두드러지는 특징이어야 해요.
두 번째 문단: 사주 구조로 왜 이런 특성이 나오는지 쉬운 말로 설명하세요.

===재물운===
(250~300자) 이 사람의 돈과의 관계를 정확하게 찌르세요.
첫 문단: 돈이 들어오는 방식의 특징 (한방형/꾸준형/말년형 중 이 사주에 맞는 것), 돈을 잃는 패턴이 있다면 정확하게. 이 사람의 사주에서 실제로 보이는 재물 특성이어야 해요.
두 번째 문단: 이 사주에서 인생에 돈이 크게 움직이는 시기가 있는지, 지금이 그 시기에 가까운지만 힌트로 주세요. 구체적인 시기는 절대 알려주지 마세요.
마지막 줄: "💰 인생 전체 재운 흐름·돈이 들어오는 구체적 시기·투자 방향은 전체 분석에서 확인하세요"
...
[중요] 무료 분석에서 이미 다룬 내용 (사주 기운 특성, 재물 패턴 힌트, 나이대별 핵심운 방향)은 절대 반복하지 마세요.
완전히 새로운 각도로만 분석하세요.

각 섹션은 ===섹션제목=== 형태로 구분하세요.

===인생 재운 흐름===
이 사람의 인생 전체에서 재물이 어떻게 흘러가는지 분석해주세요. (500~600자)
첫 문단: 인생을 3단계로 나눠서 설명하세요.
- 젊은 시절(20~30대): 돈이 어떻게 흘렀는지/흐를지
- 중년(40~50대): 재물이 가장 크게 움직이는 시기가 있는지, 있다면 언제인지 구체적으로
- 말년(60대 이후): 노후에 먹고 살기 괜찮은 사주인지 솔직하게
두 번째 문단: 이 사람이 절대 하면 안 되는 돈 실수 (보증/동업/투기 등)와 반대로 돈이 가장 잘 모이는 조건을 구체적으로 알려주세요.

===직업운 · 커리어===
(500~600자)
첫 문단: 이 사주에 가장 잘 맞는 직업군 3~4가지를 구체적으로 예시하고, 직장인/프리랜서/사업 중 어떤 방향이 맞는지 설명하세요. 이 사람의 사주에서 실제로 보이는 강점 기반으로 써주세요.
두 번째 문단: 커리어에서 크게 도약할 수 있는 시기가 있다면 구체적으로 알려주세요. 조심해야 할 시기도 포함하세요.

===투자 · 부동산===
(500~600자) 다음 4가지를 자연스러운 문장으로 모두 담아주세요:
주식 투자 성향 (단기/장기, 어떤 업종이 이 사주에 맞는지), 부동산 투자 방향 (아파트/토지/상가 등 이 사주에 맞는 유형), 실거주 환경 (어떤 방향이나 환경이 이 사주에 맞는지), 수익 파이프라인 전략 (이 사주에 맞게 수익 구조를 어떻게 나누면 좋은지).
${투자거주힌트}
...
===인간관계 · 사람운===
(400~500자)
첫 문단: 진짜 내 편이 되어줄 사람의 특징과 독이 되는 사람의 유형을 이 사주 기준으로 구체적으로 설명하세요.
두 번째 문단: 인간관계에서 이 사람이 조심해야 할 패턴과 귀인이 나타나는 상황을 설명하세요.

===월별 운세===
이 사주를 기준으로 앞으로 12개월의 운세 흐름을 한 줄씩 작성하세요.
형식: 1월: (내용) / 2월: (내용) ...
좋은 달은 왜 좋은지, 조심할 달은 구체적으로 무엇을 조심할지. 모든 달이 좋거나 나쁜 말로 채워지면 안 됩니다. 이 사주에서 실제로 보이는 흐름을 기준으로 써주세요.
...
===이 사주로 잘 사는 법===
(400~500자)
첫 문단: 이 사주의 진짜 강점과 그것을 살리는 방향을 따뜻하게 이야기하세요. 이 사람에게만 해당하는 구체적인 내용이어야 해요.
두 번째 문단: 지금 당장 실천할 수 있는 구체적 행동 조언을 주세요.`;
