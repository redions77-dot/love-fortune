...
const 오행행운아이템 = {
  '木': {
    ...
    방향: '동쪽',
    숫자: '3, 8',
    아이템: '나무 소재 액세서리, 식물 키우기, 초록색 지갑',
    설명: '목(木)의 기운이 필요해요. 성장과 생명력의 에너지예요.'
  },
  '火': {
    색깔: '빨간색, 주황색, 분홍색',
    마스코트: '말, 봉황, 불꽃 모양 소품',
    방향: '남쪽',
    숫자: '2, 7',
    아이템: '빨간 지갑, 루비·가넷 액세서리, 캔들',
    설명: '화(火)의 기운이 필요해요. 열정과 밝음의 에너지예요.'
  },
  '土': {
    색깔: '노란색, 베이지색, 황토색',
    마스코트: '소, 개, 용, 토우 인형',
    방향: '중앙',
    숫자: '5, 10',
    아이템: '황토색 가방, 도자기 소품, 원석(황수정)',
    설명: '토(土)의 기운이 필요해요. 안정과 신뢰의 에너지예요.'
  },
  '金': {
    색깔: '흰색, 은색, 금색, 회색',
    마스코트: '호랑이, 원숭이, 금속 소품',
    방향: '서쪽',
    숫자: '4, 9',
    아이템: '금·은 액세서리, 흰색 지갑, 메탈 소품',
    설명: '금(金)의 기운이 필요해요. 결단력과 귀함의 에너지예요.'
  },
  '水': {
    색깔: '검정색, 진한 파란색, 남색',
    마스코트: '쥐, 돼지, 물고기, 거북이',
    방향: '북쪽',
    숫자: '1, 6',
    아이템: '검정 지갑, 사파이어·아쿠아마린 액세서리, 물고기 소품',
    설명: '수(水)의 기운이 필요해요. 지혜와 흐름의 에너지예요.'
  },
};
...
function get월주(year, month) {
  // 월지: 1월=丑, 2월=寅 ... (인월=1월로 계산, 절기 무시 간략버전)
  const 월지index = ((month + 1) % 12);
  // 월간: 년간에 따라 결정
  const 년간index = ((year - 4) % 10 + 10) % 10;
  const 월간시작 = [2, 4, 6, 8, 0, 2, 4, 6, 8, 0]; // 년간별 1월 월간
  const 월간index = (월간시작[년간index] + (month - 1)) % 10;
...
// 음력→양력 변환 (근사치: 음력 1개월 ≈ 29.5일)
function lunarToSolar(year, month, day) {
  // 간략 변환: 음력 기준일(1900-01-31 = 음력 1900-01-01)
