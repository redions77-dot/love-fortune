...
function get시주(birthtime, 일천간index) {
  ...
  // 30분 당겨서 계산 (23:30 = 자시 시작)
  const adjustedMin = (totalMin - 30 + 24 * 60) % (24 * 60);
  const 시지index = Math.floor(adjustedMin / 120) % 12;
...
두 번째 문단: 인생에 돈이 크게 움직이는 시기가 있는지, 지금이 그 시기에 가까운지만 힌트로. 구체적 시기는 절대 알려주지 마세요.
